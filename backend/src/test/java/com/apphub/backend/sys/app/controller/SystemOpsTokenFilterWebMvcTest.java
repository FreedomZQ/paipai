...
/**
 * 针对 `SystemOpsTokenFilter` 的 WebMvc 测试。
 * 用于验证控制器路由、请求校验、响应结构以及鉴权或门禁行为是否符合预期。
 */

@WebMvcTest(value = SystemController.class, properties = "backend.ops.token=test-ops-token")
@Import({AppDefinitionService.class, AppAppleReadinessService.class})
class SystemOpsTokenFilterWebMvcTest {
    ...
    @Test
    void healthzShouldRemainPublicEvenWhenOpsTokenConfigured() throws Exception {
    ...
    @Test
    void systemEndpointsShouldRequireOpsTokenWhenConfigured() throws Exception {
    ...
    @Test
    void systemEndpointsShouldRejectWrongOpsTokenWhenConfigured() throws Exception {
        mockMvc.perform(get("/api/v1/system/apps")
    ...
    @Test
    void systemEndpointsShouldPassWhenOpsTokenMatches() throws Exception {
        mockMvc.perform(get("/api/v1/system/apps")
