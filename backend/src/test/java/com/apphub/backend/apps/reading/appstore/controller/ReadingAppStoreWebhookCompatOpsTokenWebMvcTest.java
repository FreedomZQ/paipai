...
/**
 * 针对 `ReadingAppStoreWebhookCompatOpsToken` 的 WebMvc 测试。
 * 用于验证控制器路由、请求校验、响应结构以及鉴权或门禁行为是否符合预期。
 */

@WebMvcTest(value = ReadingAppStoreWebhookCompatController.class, properties = "backend.ops.token=test-ops-token")
@Import(AppCompatControllerSupport.class)
class ReadingAppStoreWebhookCompatOpsTokenWebMvcTest {
