...
- `Local Guest`：本地匿名状态，无需家长登录即可使用，不上传儿童内容，不采集诊断，不已下线多端数据能力，后端停止服务时仍可继续使用本地能力。
- `Parent Account`：家长授权状态，Apple 登录和家长同意后才开启云 OCR/TTS、已下线多端数据能力、购买恢复、补偿兑换和后端权威权益。
- 账号层只保留家长身份，不采集孩子身份。
- 本地 OCR / 语音默认离线可用。
...
- iOS 侧 `BackendRoute` 已经有 appCode-scoped system auth / 已下线多端数据方案 路由，可继续作为家长授权态的路由边界。
...
1. 前端现有 `storageScope = authSession?.account.accountId ?? "signed-out"`、本地仓库、已下线多端数据方案 仓库和 `AppScopedDefaults` 已经具备 signed-out 本地使用基础；后续不得把 `signed-out` 当成服务端匿名账号，也不得在 Local Guest 下 bootstrap 已下线多端数据方案 或上传诊断。
...
4. 现有 `reading_child_profile`、`reading_review_card`、`reading_usage_session`、`reading_review_event` 等表以 `user_id` 关联孩子和学习数据；改造后这些表只能作为 Parent Account 已下线多端数据能力数据使用，并且必须受同意、删除、导出、撤回同意控制。Local Guest 的孩子档案、句卡、学习记录只能落本地库。
...
- 进入云能力、已下线多端数据能力、补偿、购买恢复、账号删除前，自动弹出 `Parent Account` 授权流程，无需用户手动切换账户。
...
- `已下线多端数据方案InstallationStore.installationId(scope:)` 只能用于已下线多端数据能力安装状态，不得复用为 Local Guest 设备身份、权益身份或诊断身份；删除账号或关闭已下线多端数据能力时必须清空对应 scope 的同步凭证和安装记录。
...
- 访问权：家长可查看账号、同意范围、权益余额、已下线多端数据能力状态、第三方云能力开关。
...
当前项目已有删除账号入口、购买页、统一计费和权益中心，但计划文档必须明确“购买/账号数据是否可删除”的边界：儿童内容和已下线多端数据能力数据应删除；支付、税务、反欺诈、退款争议所必需的最小购买凭证不能直接物理删除，应去标识化或隔离保留到必要期限。
...
- 立即删除或软删除：儿童档案、学习记录、句卡、同步安装记录、已下线多端数据能力数据、诊断日志、匿名权益 token、未使用的补偿 claim。
- 终止或匿名化：`sys_auth_session`、Apple provider token、已下线多端数据方案 token、远程配置个性化状态。
...
- 已下线多端数据方案 或其他同步供应商，如果启用已下线多端数据能力。
...
- `reading_child_profile`、`reading_review_card`、`reading_review_event`、`reading_usage_session`、`reading_weekly_report_snapshot` 标记为 Parent Account 已下线多端数据能力数据，新增 `privacy_case_id`、`deleted_at` 或删除任务关联字段；Local Guest 对应数据不进这些表。
...
- `/removed-data-feature-api/{appCode}/bootstrap`、`/removed-data-feature-api/{appCode}/token`、`/removed-data-feature-api/{appCode}/upload` 只能在 Parent Account + sync consent 后启用；Local Guest 本地库不得请求这些接口。
...
- 补偿、购买恢复、已下线多端数据能力、账号删除：提示后端服务恢复后再处理；账号删除请求可先在本地排队，但不得声称云端已删除。
...
| `front/ios/PaipaiReadAlong/App/PaipaiReadAlongApp.swift` | 支持 signed-out 本地使用；云 OCR/TTS、已下线多端数据能力、购买恢复、补偿、账号删除触发时再要求家长授权 |
...
| `front/ios/PaipaiReadAlong/Core/Repositories/ChildProfileRepository.swift`、`ReviewCardRepository.swift`、`UsageSessionRepository.swift`、`ReviewEventRepository.swift` | signed-out 本地数据不得上传；登录后只有 sync consent 才能进入 已下线多端数据方案 |
| `front/ios/PaipaiReadAlong/Core/Sync/已下线多端数据方案Connector.swift`、`已下线多端数据方案BootstrapAPI.swift`、`已下线多端数据方案UploadAPI.swift`、`已下线多端数据方案InstallationStore.swift`、`已下线多端数据方案CredentialStore.swift` | 仅 Parent Account + sync consent 启用；删除账号或撤回同步同意后清理本地同步凭证和安装 ID |
| `front/ios/PaipaiReadAlong/Features/Parent/SyncSettingsView.swift` | 已下线多端数据能力默认关闭，开启前走家长授权、direct notice、granular consent |
...
- `OnboardingView`、`PaywallView`、`DeleteAccountView`、`EntitlementRecordsView` 文案先与实际能力对齐：不承诺强制已下线多端数据能力、不承诺删除全部购买凭证、不把本地权益描述成云端权益。
...
15. 删除账号后，App Store 后续通知不得重建账号、儿童数据、权益 token 或已下线多端数据能力状态。
