...
1. 后端当前仅保留拍拍伴读运行所需的业务模块和通用系统能力。
2. 新建表从现在开始严格遵循：
   - `sys_`：通用系统表
   - `reading_`：拍拍伴读业务表
3. 技术栈：Java 17 + Spring Boot 3 + MyBatis Plus + Maven + PostgreSQL + Redis + YAML + dev/test/prod
4. 后端持久层使用 **MyBatis Plus**。
...
- Entitlement / product mapping 可配置化已落地并测试通过：
  ...
  - 这样同一个 entitlement 可以映射多个 App Store productId，避免把 productId 当业务权益硬编码
  ...
  - 聚合门禁接口：`/api/v1/system/apple/ops-gates`，用于 CI/CD 或上线脚本一次性检查 supported apps
  ...
  - 默认 seed 已包含拍拍伴读的 `billing_entitlements` 与 `billing_refresh_policy` 样板，便于新环境起库后直接观测和调整
- 拍拍伴读兼容路由已落地并测试通过：
...
- System / ops 接口安全收口已落地并测试通过：
  ...
  - 故意保持公开的入口已归档到 `GET /api/v1/system/public-surface`：包括 system healthz、actuator health、appCode-scoped Apple exchange，以及 reading 的 App Store webhook 兼容路由；system demo session 默认关闭，仅在 app 显式 `app.auth.demoSessionEnabled=true` 时进入公开面清单
...
- 公开 auth/bootstrap 策略已收口：reading 仅保留 Apple exchange 作为正式登录入口且 `demoSessionEnabled=false`；system demo session 默认关闭，Apple-only app 开启会进入 ops-gate blocker；对应状态会进入 ops-gate / release-gate
